...
object CodeGen {
  ...
  def generateCodeSafe(
    ...
    val errorsOrUnits: ValidationNel[String, List[Unit]] = files.traverseU { f =>
      generateCodeImpl(parsePayload(reader))(f, packageName, outputDir, mode).validationNel
    }

    errorsOrUnits.map(_ => ())
  }

  private def generateCodeImpl(parse: File => String \/ Dar[Payload])(
      input: File,
      packageName: String,
      outputDir: File,
    ...
    for {
      dar <- parse(input)
      iface <- decodeInterface(dar)
    } yield packageInterfaceToScalaCode(util(mode, packageName, iface, outputDir))
